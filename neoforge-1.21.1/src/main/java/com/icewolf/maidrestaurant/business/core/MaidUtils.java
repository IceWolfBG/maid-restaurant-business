...
public class MaidUtils {
        ...
        try {
            ...
            for (EntityMaid maid : allMaids) {
                ...
                try {
                    ...
                    net.neoforged.neoforge.items.IItemHandler inv = maid.getAvailableInv(false);
                    if (inv != null) {
                    ...
                    if (foundMachine == null) {
                        try {
                            ...
                            if (baubleHandler instanceof net.neoforged.neoforge.items.IItemHandler) {
                                net.neoforged.neoforge.items.IItemHandler baubleInv = (net.neoforged.neoforge.items.IItemHandler) baubleHandler;
    ...
    private static boolean isMaidActuallyWorking(EntityMaid maid, ServerLevel level) {
        ...
        // 1. TaskManager 活跃任务（烹饪任务经 createTask 登记，内部还会按 CookRequest 续期）
        try {
        ...
        // 7a. 女仆餐厅烹饪大脑信号：目标厨具位 / 餐椅位（反射读取，避免 DeferredHolder 泛型耦合）
        try {
            Class<?> modEntities = Class.forName("com.mastermarisa.maid_restaurant.init.ModEntities");
            for (String name : new String[]{"TARGET_POS", "CHAIR_POS"}) {
                try {
                    java.lang.reflect.Field f = modEntities.getDeclaredField(name);
                    f.setAccessible(true);
                    Object memory = f.get(null);
                    if (memory instanceof MemoryModuleType
                        && maid.getBrain().hasMemoryValue((MemoryModuleType<?>) memory)) {
                        return true;
                    }
    ...
    public static int countItem(IItemHandler inv, ResourceLocation itemId) {
        ...
        for (int i = 0; i < inv.getSlots(); ++i) {
            ResourceLocation rl;
            ItemStack stack = inv.getStackInSlot(i);
            if (stack.isEmpty() || !itemId.equals((Object)(rl = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stack.getItem())))) continue;
    ...
    public static int takeItem(IItemHandler from, IItemHandler to, ResourceLocation itemId, int maxCount) {
        ...
        for (int i = 0; i < from.getSlots() && taken < maxCount; ++i) {
            ...
            if (stack.isEmpty() || !itemId.equals((Object)(rl = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stack.getItem()))) || (extracted = from.extractItem(i, toTake = Math.min(stack.getCount(), maxCount - taken), false)).isEmpty()) continue;
