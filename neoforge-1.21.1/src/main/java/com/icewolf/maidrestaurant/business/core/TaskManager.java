...
public class TaskManager {
    ...
    public List<BlockPos> getCachedCountersWithPlates(ServerLevel level) {
        if (currentTick - lastPlateCacheTick >= PLATE_CACHE_INTERVAL) {
            updateCountersWithPlatesCache(level);
        }
    ...
    /**
     * 更新带餐盘的操作台缓存（中心化检索）
     * 扫描所有加载区块中的操作台，检查周围是否有餐盘
     */
    private void updateCountersWithPlatesCache(ServerLevel level) {
        ...
        try {
            ...
            for (BlockPos machinePos : activatedMachines) {
                ...
                for (BlockPos checkPos : BlockPos.betweenClosed(
                    ...
                    if (result.contains(counterPos)) continue;
                    // 检查操作台周围是否有餐盘
            ...
            MaidRestaurantBusiness.LOGGER.warn("TaskManager: 更新餐盘缓存异常", t);
        }
