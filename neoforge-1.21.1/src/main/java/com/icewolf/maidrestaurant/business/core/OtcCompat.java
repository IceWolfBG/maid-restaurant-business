...
/**
 * Order to Cook 兼容工具类
 * 同时支持 Forge 版本（DeferredHolder<Item, Item>）和 Fabric 版本（直接 Item）
...
public class OtcCompat {
    ...
    /**
     * 通过反射获取 ModItems 字段的值，自动处理 DeferredHolder 和直接 Item
     */
    private static Item getModItem(String fieldName, String fallbackId) {
        ...
        try {
            ...
            // Forge 版本：DeferredHolder<Item, Item>
            if (value instanceof net.neoforged.neoforge.registries.DeferredHolder) {
                Object item = ((net.neoforged.neoforge.registries.DeferredHolder<?, ?>) value).get();
    ...
    private static Item getItemById(String id) {
        return net.minecraft.core.registries.BuiltInRegistries.ITEM.get(ResourceLocation.tryParse(id));
    }
